...
Require-Match $material 'corduroy_amount"\s*,\s*0\.018' "Groomed corduroy must remain subtle enough to avoid foreground striping."
...
Require-Match $tracks 'ROUGHNESS\s*=\s*mix\(0\.52,\s*0\.92,\s*skid\)\s*\*\s*mix\(1\.0,\s*0\.78,\s*carve\)' "Carved and disturbed ski tracks must have distinct reflection responses."
...
Require-Match $snowVfx 'var particle_mesh := SphereMesh\.new\(\)' "Snow particles must use rounded geometry instead of square billboard draw passes."
Require-Match $snowVfx 'particle_mesh\.radial_segments\s*=\s*8' "Snow particle geometry must stay low-poly and inexpensive."
Reject-Match $snowVfx 'var quad := QuadMesh\.new\(\)' "Snow particles cannot use a hard-edged quad draw pass."
