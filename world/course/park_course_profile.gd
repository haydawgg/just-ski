...
func spot_specs() -> Array[ParkSpotSpec]:
	var spots: Array[ParkSpotSpec] = [
		...
		ParkSpotSpec.create(
			&"technical_yard", "Technical Yard", Vector3(2.0, 0.0, 47.0), Vector3(0.0, 0.0, 72.0),
			[&"technical", &"jib", &"terrain"], _feature_ids(["MediumTable", "MidMoguls", "MidButterPad", "KinkRail", "DFDBox", "MidWallride"])
		),
		ParkSpotSpec.create(
			&"transfer_zone", "Transfer Zone", Vector3(3.0, 0.0, 2.0), Vector3(0.0, 0.0, 35.0),
			[&"transfer", &"side_hit", &"line_choice"], _feature_ids(["HipTransfer", "CenterSpine", "MidRoller", "LongTube", "TransferBox", "TransferBonk"])
		),
		ParkSpotSpec.create(
			&"lower_hero", "Lower Hero and Rainbow", Vector3(5.0, 0.0, -49.0), Vector3(0.0, 0.0, -20.0),
			[&"hero", &"rainbow", &"progression"], _feature_ids(["LargeTable", "LowerRightSideHit", "LowerButterPad", "SRail", "Rainbow", "LowerWallride"])
		),
		ParkSpotSpec.create(
			&"finale", "Finale", Vector3(2.0, 0.0, -111.0), Vector3(0.0, 0.0, -78.0),
			[&"finale", &"commitment", &"runout"], _feature_ids(["StepDownTable", "FinalCannon", "FinalBox", "FinalDFDRail", "FinalCatchBerm", "FinishGate"])
		),
	]
...
func _content_placement_for(feature_name: String) -> Dictionary:
	var spot_by_feature := {
		...
		"StepDownTable": &"finale", "FinalCannon": &"finale", "FinalBox": &"finale", "FinalDFDRail": &"finale", "FinalCatchBerm": &"finale", "FinishGate": &"finale",
	}
	var safe := ["SummitStartGate", "SummitRollerA", "SummitRollerB", "UpperRoller", "UpperBermLeft", "UpperBermRight", "MidMoguls", "MidButterPad", "MidRoller", "LowerButterPad", "FinalBox", "FinalCatchBerm", "FinishGate"]
