...
func _on_telemetry(data: Dictionary) -> void:
	...
	debug_label.text += "\nOwner %s  %s→%s %.2f  legIK %.2f preview %.2f pelvis %s" % [
		...
		animation.get("leg_ik_weight", 0.0), animation.get("air_preview_ik_weight", 0.0), animation.get("pelvis_ik_correction", Vector3.ZERO)]
