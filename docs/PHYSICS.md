...
All important motion values live in `SkiPhysicsProfile`: ground/air gravity, per-surface response, glide, pressure, forward/lateral friction, edge set/release, sidecut, weathervane, high/low-speed steering, heading/travel limits, analog braking, drag/tuck, jump charge/pop, coyote/min air time, terminal speed, air damping/landing prediction, landing retention/gates, bail recovery, and rail capture/balance/exit values. F3 displays raw and shaped steering, effective steer rate, slope angle, heading/travel separation, brake amount, carve/skid ratio, grip versus centripetal demand, contact data, angular velocity, and rail state.
