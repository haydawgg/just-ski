...
`BAIL` is a hybrid crash state. Normal skiing remains controller driven, but a committed crash transfers the evaluated pose, root linear velocity, and world angular velocity into a crash-only articulated rigid-body proxy. The proxy uses pelvis/chest/head, upper/lower limbs, bounded joints, continuous collision detection, torso-heavy mass distribution, and a low-restitution snow material. A short high-damping active phase resists the initial impact before the bodies become passive. The normal skiing orientation and locomotion solvers do not run while the ragdoll owns motion.

Ski and pole ownership is decided independently from the body. Soft crashes keep each ski rigidly bound to its shin and each pole with its forearm. Impact/twist severity can release one or both skis, while serious crashes release the cosmetic pole bodies immediately. Released equipment collides with terrain but not the player skeleton. Ski debris receives weaker lateral resistance than an attached ski; longitudinal resistance remains low. The ordinary skiing model continues to use its established four-point ski probes and anisotropic carve/skid forces.

Recovery waits for a sufficiently quiet grounded rest state or the severity-scaled settle deadline, freezes the physical pose, and blends it into the staged get-up while gameplay remains in `BAIL`. The gameplay root is reseated and aligned to the live snow plane only when that blend completes. `GROUND` then resumes with a short crash-immunity window so the get-up capsule cannot immediately retrigger the impact that just ended. An unsupported airborne bail that reaches its maximum duration still uses normal marker/default respawn.
