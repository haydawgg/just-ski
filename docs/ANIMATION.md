...
Bail animation is a staged procedural fallback plus a crash-only physical-pose presentation. `RELEASE`, `IMPACT`, `FALL`, `REST`, and `RECOVERY` remain the public phase contract, but a committed runtime crash normally hands the final evaluated skeleton/equipment transforms to the articulated rigid-body proxy. The animation controller consumes that world-space physics pose after its ordinary frame evaluation, so normal ski/air layers cannot fight the ragdoll. Test and tooling paths can disable the profile-owned ragdoll flag and retain the procedural fallback.

The animation controller reads the captured crash context and produces the same stages using stage-local progress. Before physics activation, the crash reaction layer provides the release pose and preserves a deterministic procedural fallback. During physical `FALL`/`REST`, the rig adapter maps rigid-body transforms back through the canonical semantic joints and then into the production skeleton. Bound equipment follows its limb body through a captured rigid offset; released skis and poles receive independent world transforms after bone attachments update. `RECOVERY` freezes the final physical pose and fades its weight to zero while the authored get-up recenters the body and reacquires contact. Recovery completion then enters ground presentation; respawn remains a separate hard-reset path.
