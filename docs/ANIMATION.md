...
The runtime rig contains independent joints for pelvis, spine, chest, head, hips, knees, boots, skis, shoulders, elbows, hands, and poles. The visible model includes a jacket, pants, helmet, goggles, gloves, boots, widened skis, thicker poles, and readable pole baskets. Slightly wider hips and slimmer leg primitives preserve negative space between the legs at gameplay distance. All meshes are Godot primitives, so no third-party asset license is involved.
...
This is deliberately a controlled procedural fall for the current primitive articulated rig. It does not add a `RigidBody3D`, `PhysicalBoneSimulator3D`, or a second gameplay state machine. A future production skeleton can replace the adapter behind the same frame/context seam.
